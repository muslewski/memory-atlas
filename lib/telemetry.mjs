...
export function resolveTelemetryEnabled(opts = {}) {
  ...
  const repo = opts.repoConfig
  const rTel = repo?.telemetry
  if (rTel && typeof rTel === 'object' && typeof rTel.enabled === 'boolean') {
    return rTel.enabled
  }
...
export function runTelemetry(argv, opts = {}) {
  ...
  try {
    if (sub === 'status') {
      const enabled = resolveTelemetryEnabled({ env, argv: process.argv })
      const events = readEvents({ eventsPath })
      const last = events[events.length - 1]
      const id = enabled || events.length ? getInstallId().slice(0, 8) : '(none)'
      stdout.write(
        `atlas telemetry: ${enabled ? 'ON' : 'OFF'} (default off; fleet: ~/.config/memory-atlas/config.json)\n`,
      )
