...
export function makeResolvers(repoRoot, anchorsConfig = {}) {
  const resolvers = {
    ...
    // Expand positive globs to tracked files. Single git call so
    // `:(exclude)…` pathspecs subtract; a lone exclude (no positive) would
    // otherwise match the whole repo — return [] instead.
    filesFor: (globs) => {
      const list = globs ?? []
      const positives = list.filter((g) => g.trim() && !isExcludePathspec(g))
      if (positives.length === 0) return []
      try {
        const out = execFileSync('git', ['ls-files', '--', ...list], {
          cwd: repoRoot,
          encoding: 'utf8',
        })
        return [
