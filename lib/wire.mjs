...
/**
 * Install a local git merge driver that regenerates map/index.md on conflict.
 * Per-clone: .gitattributes is committed; `git config merge.atlas-index.*` is not.
 *
 * @param {string} repoRoot
 * @param {{ stdout: { write: Function }, stderr: { write: Function }, log: (s: string) => void }} io
 * @returns {number}
 */
export function wireMergeDriver(repoRoot, io) {
  ...
  const attrLine = `${vaultRel}/map/index.md merge=atlas-index`
  ...
  if (existsSync(attrPath)) {
    ...
    if (existing.split('\n').some((l) => l.trim() === attrLine || l.includes('merge=atlas-index'))) {
      log(`.gitattributes: already wired (${attrLine})`)
    } else {
      const next = existing.endsWith('\n') || existing === '' ? existing : existing + '\n'
      writeFileSync(attrPath, `${next}${attrLine}\n`)
      attrChanged = true
      log(`.gitattributes: added ${attrLine}`)
    }
  } else {
    writeFileSync(attrPath, `${attrLine}\n`)
    attrChanged = true
    log(`.gitattributes: created with ${attrLine}`)
  }

  // Prefer absolute path to this package's bin so the driver works without PATH.
  const atlasBin = path.join(PACKAGE_ROOT, 'bin', 'atlas.mjs')
  const driver = `node ${JSON.stringify(atlasBin)} merge-index %O %A %B %L %P`

  const nameR = spawnSync(
    'git',
    ['config', 'merge.atlas-index.name', 'regenerate Atlas map/index.md'],
    { cwd: repoRoot, encoding: 'utf8' },
  )
  const drvR = spawnSync('git', ['config', 'merge.atlas-index.driver', driver], {
    cwd: repoRoot,
    encoding: 'utf8',
  })
  if (nameR.status !== 0 || drvR.status !== 0) {
    stderr.write(
      `atlas wire merge-driver: git config failed: ${(nameR.stderr || drvR.stderr || '').trim()}\n`,
    )
    return 1
  }
  log('git config: merge.atlas-index.driver set (local to this clone)')
  log(`  driver: ${driver}`)
  log('note: git merge driver config is per-clone — re-run `atlas wire merge-driver` after clone')
  if (!attrChanged) {
...
export function runWire(argv, opts = {}) {
  ...
  if (target === 'merge-driver') {
    return wireMergeDriver(repoRoot, { stdout, stderr, log })
  }
