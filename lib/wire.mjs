 ...
 * pristine; leave locally-edited copies alone), unless the skill is already
 * installed at user scope (see lib/skills.mjs — ATLAS_USER_SKILLS_DIR).
 */
...
/**
 * Record that `name` is satisfied at user scope (no repo copy required).
 * Clears any prior repo-path claim so doctor does not look for a missing file.
 *
 ...
 */
function recordUserScopeSkill(state, name, userHash, version, log, userRoot) {
  const vendoredKey = `skills/${name}/SKILL.md`
  const next = userScopeVendoredEntry(userHash, version)
  ...
  if (
    ...
    prev.source !== SOURCE_USER_SCOPE
  ) {
    state.vendored[vendoredKey] = next
    stateDirty = true
    // First transition into user-scope bookkeeping is an action worth logging.
    if (!prev || prev.source !== SOURCE_USER_SCOPE) {
      log(`skill ${name}: user-scope satisfies (${path.join(userRoot, name)})`)
      anyAction = true
    }
  }
 ...
 *
 * When the same skill is present at user scope and `skills.vendorInRepo` is
 * not true, skip the copy and record `source: "user-scope"` in state.
 *
 * @param {string} repoRoot
 ...
 * @returns {{ anyAction: boolean, stateDirty: boolean }}
 */
function vendorSkillsFrom(repoRoot, skillsDir, srcRoot, state, log, config, skillOpts) {
  let anyAction = false
  let stateDirty = false
  const destRoot = path.join(repoRoot, skillsDir)
  const version = packageVersion()
  const userRoot = resolveUserSkillsDir(skillOpts)
  const forceRepo = wantsVendorInRepo(config)

  for (const name of listSkillsIn(srcRoot)) {
    const vendoredKey = `skills/${name}/SKILL.md`

    // User-scope satisfaction: do not vendor into the repo.
    if (!forceRepo && skillPresent(userRoot, name)) {
      const userHash = hashSkillFile(skillMdPath(userRoot, name))
      if (userHash) {
        const r = recordUserScopeSkill(state, name, userHash, version, log, userRoot)
        if (r.anyAction) anyAction = true
        if (r.stateDirty) stateDirty = true
        continue
      }
    }
    ...
    if (!destExists) {
      shouldCopy = true
    } else if (prev?.sha256 && prev.source !== SOURCE_USER_SCOPE) {
    ...
    } else if (prev?.source === SOURCE_USER_SCOPE && destExists) {
      // State said user-scope but a repo copy appeared (or vendorInRepo flipped on).
      // Treat existing dest as foreign unless hashes match recorded user hash and
      // we are forcing vendor — then overwrite only when forceRepo.
      if (forceRepo) shouldCopy = true
    }
    // dest exists, no recorded hash: treat as foreign/local — leave alone

    const nextVendored = repoVendoredEntry(upstreamHash, version)
    ...
    if (shouldCopy) {
      ...
      if (!alreadyCurrent) {
        ...
        else if (contentChanged || prev?.source === SOURCE_USER_SCOPE) log(`updated skill ${name}`)
        anyAction = true
      }
      ...
      if (
        !p ||
        p.sha256 !== nextVendored.sha256 ||
        p.atlasVersion !== nextVendored.atlasVersion ||
        p.source !== nextVendored.source
      ) {
 ...
 * @returns {{ anyAction: boolean, stateDirty: boolean }}
 */
function vendorSkills(repoRoot, skillsDir, state, log, config, skillOpts) {
  return vendorSkillsFrom(repoRoot, skillsDir, PACKAGE_SKILLS_DIR, state, log, config, skillOpts)
}
 ...
 * @returns {{ anyAction: boolean, stateDirty: boolean }}
 */
function vendorVisualsSkills(repoRoot, skillsDir, state, log, config, skillOpts) {
  let anyAction = false
  let stateDirty = false
  ...
  if (existsSync(stubsRoot)) {
    const stubResult = vendorSkillsFrom(
      repoRoot,
      skillsDir,
      stubsRoot,
      state,
  ...
  if (existsSync(peerSkills)) {
    const peerResult = vendorSkillsFrom(
      repoRoot,
      skillsDir,
      peerSkills,
      state,
...
export function runWire(argv, opts = {}) {
  ...
  // unless user-scope already satisfies (see lib/skills.mjs).
  const skillResult = vendorSkills(repoRoot, skillsDir, state, log, config, skillOpts)
  ...
  const visualsResult = vendorVisualsSkills(
    repoRoot,
    skillsDir,
    state,
    log,
