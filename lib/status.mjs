...
import { packageVersion, readState } from './state.mjs'
...
export function runStatus(argv, opts = {}) {
  ...
  try {
    ...
    // Offline update nudge: installed package vs wired lockfile version.
    const state = readState(repoRoot)
    if (state?.atlasVersion && state.atlasVersion !== packageVersion()) {
      const skillsDir =
      ...
      stdout.write(
        `⬆ atlas ${installed} installed, wired ${state.atlasVersion} — run the atlas-update skill (${skillsDir}/atlas-update/SKILL.md)\n`,
      )
    }
    return 0
  } catch {
