...
export function validate(zones, flows, r, graph = {}) {
  ...
  for (const z of zones) {
    ...
    if (routes.length > 0) {
      if (typeof r.route === 'function') {
        for (const rt of routes) {
          if (!r.route(rt)) warnings.push(`zone ${z.id}: route "${rt}" not confidently resolved — verify`)
        }
      } else {
    ...
    // SPEC.md's verifiedAt rule now covers `unmounted` zones: a verifier
    // MUST NOT run the staleness check against them (their anchors point at
    // code that no longer exists) and MUST NOT error on either legal
    // encoding they carry. So: attic them BEFORE any encoding/staleness
    // check runs — never after.
 ...
 * @returns {string}
 */
export function renderIndex({ rows = [], warnings = [], graphWarnings = [], attic = [] } = {}) {
