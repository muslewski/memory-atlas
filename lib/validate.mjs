...
import { findOwnershipConflicts } from './corpus.mjs'
 ...
 * @property {(globs: string[]) => string[]} [filesFor] expand owns.globs to tracked files (ownership SSOT)
 ...
 * @param {Resolvers} r
 * @param {{ noteIds?: Set<string>, pillars?: Array<Record<string, unknown>>, decisions?: Array<Record<string, unknown>>, check?: { ownership?: boolean, strictFreshness?: boolean, corpus?: Record<string, unknown> }, profile?: string }} [graph]
...
export function validate(zones, flows, r, graph = {}) {
  ...
  for (const z of zones) {
    ...
    const globs = anchorArray(owns, 'globs')
    // Profile-aware: `code` (default) requires ≥1 glob; `operator` may have
    // policy/ledger zones with empty globs (warning only — not a hard fail).
    const profile = graph.profile === 'operator' ? 'operator' : 'code'
    if (globs.length === 0) {
      if (profile === 'code') {
        errors.push(
          `zone ${z.id}: owns.globs is empty — code profile requires at least one pathspec (or set profile: operator)`,
        )
      } else {
        warnings.push(
          `zone ${z.id}: owns.globs empty (operator profile — policy zone; no code ownership claim)`,
        )
      }
    }
    ...
    for (const inv of z.invariants ?? []) {
      // Flow-style list items parse as bare strings (frontmatter subset does
      // not support `{ rule: ... }` maps), so rule is missing. Name that
      // problem instead of warning about invariant "undefined".
      const rule =
        inv !== null && typeof inv === 'object' && typeof inv.rule === 'string'
          ? inv.rule
          : undefined
      if (rule === undefined) {
        warnings.push(
          `zone ${z.id}: invariant missing rule field (flow-style YAML maps are not supported — use block style)`,
        )
        continue
      }
      const enforcedBy = Array.isArray(inv.enforcedBy) ? inv.enforcedBy : []
      if (enforcedBy.length === 0) {
        warnings.push(`zone ${z.id}: invariant "${rule}" has no enforcedBy → file tech-debt`)
      }
    }

    const verifiedAt = z.verifiedAt
    const encodingOk = isUnverified(verifiedAt) || isSha(verifiedAt)
    if (z.status === 'seeded') {
      ...
      // active + unverified is legal after a stamp-only merge invalidation
      // (neither parent SHA is honest for the merged tree). See decision
      // 2026-07-30-verifiedAt-after-merge-unverified. ISO dates / empty still fail.
      if (!encodingOk) {
        errors.push(
          `zone ${z.id}: status "active" requires a commit SHA or "unverified" for verifiedAt, found "${verifiedAt}"`,
        )
      }
    } else if (!encodingOk) {
      errors.push(
        `zone ${z.id}: status "${z.status}" requires a commit SHA or "unverified" for verifiedAt, found "${verifiedAt}"`,
      )
    }

    // Freshness must never disagree with encoding validation: a rejected
    // stamp (ISO date, empty, garbage) must not render as Freshness "ok".
    let freshness
    if (isUnverified(verifiedAt)) {
      freshness = 'seeded'
      if (z.status === 'active') {
        warnings.push(
          `zone ${z.id}: active with verifiedAt unverified — re-stamp after merge or review`,
        )
      }
    } else if (!isSha(verifiedAt)) {
      // Invalid encoding (ISO date, empty, other garbage) — already an error
      // above when status requires a legal stamp. Index row stays honest.
      freshness = '⚠ invalid'
    } else {
  ...
  // Date-prefixed ids (YYYY-MM-DD-…) are not ADR numbers — require the fourth
  // digit group to NOT look like a month (01–12) continuation. Match only
  // classic ADR ids: NNNN-slug where NNNN is not the start of an ISO date
  // (i.e. not followed by -MM-DD).
  const decisionsByNum = new Map()
  for (const d of decisions) {
    const id = String(d.id ?? '')
    // Skip YYYY-MM-DD-… date prefixes (would false-positive as number "2026").
  ...
  // Ownership SSOT: one artifact, one owner. Hard error (correctness invariant).
  // Runs when the caller injected filesFor AND check.ownership is not opted out.
  if (typeof r.filesFor === 'function' && check.ownership !== false) {
    for (const v of findOwnershipConflicts(zones, r.filesFor)) {
      errors.push(`zone ${v.zoneId}: ${v.message}`)
    }
  }
