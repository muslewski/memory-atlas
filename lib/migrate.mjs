...
export function runMigrate(argv, opts = {}) {
  ...
  if (pending.length === 0) {
    // Patch releases often ship zero FS migrations. Still advance the lockfile
    // so package-freshness wired tier matches installed after `migrate --write`.
    // Only stamp when installed is strictly newer than wired (never rewrite a
    // fixture/ahead-of-registry atlasVersion downward).
    const wired = state?.atlasVersion ?? null
    const needsStamp =
      write && (wired == null || compareVersions(String(wired), installed) < 0)
    if (needsStamp) {
      const next = state ? { ...state } : defaultState()
      next.atlasVersion = installed
      writeState(repoRoot, next)
      if (json) {
        log(JSON.stringify({ installed, wired: installed, pending: [], stamped: true }))
      } else {
        log(`✓ stamped atlasVersion ${wired ?? '∅'} → ${installed} (no migrations)`)
      }
      return 0
    }
    if (json) {
