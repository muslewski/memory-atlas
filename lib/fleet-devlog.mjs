...
/**
 * Path to the fleet-devlog reference module for drift checks.
 * No desk-absolute default: published packages must not embed host paths.
 * Set FLEET_DEVLOG_REF in the environment, or the caller gets null and
 * must treat "unresolved" as a loud failure (not a silent skip).
 *
 * @param {NodeJS.ProcessEnv} [env]
 * @returns {string | null}
 */
export function referencePath(env = process.env) {
  const p = env.FLEET_DEVLOG_REF
