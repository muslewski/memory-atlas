 ...
 * plus re-vendor skill findings (redundant / drift vs user-scope).
 *
 * Always prints findings when present. Exit 1 only when mode is fail or
 * `--strict` is set AND there are package-freshness issues (re-vendor
 * findings are always soft — they need a human choice). Fail-open on
 * unexpected errors unless --strict (then exit 1).
 */

import { loadConfig } from './config.mjs'
import { findRepoRoot } from './detect.mjs'
import { computePackageFreshness, shouldExitNonZero } from './package-freshness.mjs'
import { collectRevendorFindings, resolveSkillsDir } from './skills.mjs'
...
export function runGate(argv, opts = {}) {
  ...
  try {
    ...
    const hasFreshness = report.messages.length > 0
    const hasRevendor = revendor.length > 0

    if (!hasFreshness && !hasRevendor) {
