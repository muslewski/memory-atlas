 ...
 * @param {{ userSkillsDir?: string, env?: NodeJS.ProcessEnv, homedir?: () => string }} [skillOpts]
 */
function reportVisuals(repoRoot, config, skillsDir, log, skillOpts = {}) {
  ...
  const userRoot = resolveUserSkillsDir(skillOpts)
  for (const name of VISUALS_SKILLS) {
    const skillPath = path.join(repoRoot, skillsDir, name, 'SKILL.md')
    if (existsSync(skillPath)) {
      log(`✓ skills/${name}: present`)
    } else if (skillPresent(userRoot, name)) {
      log(`✓ skills/${name}: user-scope (${skillMdPath(userRoot, name)})`)
    } else {
...
/**
 * Map vendored key → absolute path under repoRoot (or user-scope for skills).
 * - `CLAUDE.md#atlas:onramp` → `<root>/CLAUDE.md`
 * - `skills/<name>/SKILL.md` → repo or user-scope path depending on meta.source
 *
 * @param {string} repoRoot
 * @param {string} key
 * @param {string} [skillsDir]
 * @param {{ sha256?: string, source?: string } | null} [meta]
 * @param {{ userSkillsDir?: string, env?: NodeJS.ProcessEnv, homedir?: () => string }} [skillOpts]
 * @returns {string | null}
 */
function vendoredPath(repoRoot, key, skillsDir = '.claude/skills', meta = null, skillOpts = {}) {
  if (key.startsWith('skills/')) {
    return resolveVendoredSkillPath(repoRoot, key, skillsDir, meta, skillOpts)
  }
 ...
 *   homedir?: () => string,
 * }} [opts]
 * @returns {number} 0 normally; 1 when --strict and package-freshness issues
 */
export function runDoctor(argv = [], opts = {}) {
  ...
  try {
    ...
    if (!state) {
      ...
      for (const [key, meta] of Object.entries(vendored)) {
        const filePath = vendoredPath(repoRoot, key, skillsDir, meta, skillOpts)
        const content = filePath ? vendoredContent(filePath, key) : null
        if (!content) {
          if (key.startsWith('skills/') && meta?.source === SOURCE_USER_SCOPE) {
            log(`✗ ${key}: missing at user-scope`)
          } else {
            log(`✗ ${key}: missing`)
          }
          continue
        ...
        if (recorded && diskHash === recorded) {
          if (meta?.source === SOURCE_USER_SCOPE) {
            log(`✓ ${key}: user-scope pristine`)
          } else {
            log(`✓ ${key}: pristine`)
          }
        } else if (meta?.source === SOURCE_USER_SCOPE) {
          log(`⚠ ${key}: user-scope content differs from recorded hash`)
        } else {
    ...
    const skillsDir = resolveSkillsDir(config)
    reportVisuals(repoRoot, config, skillsDir, log, skillOpts)

    // Re-vendor check: repo copy present while user-scope already has the skill
    const revendor = collectRevendorFindings(repoRoot, skillsDir, config, skillOpts)
    for (const f of revendor) {
      for (const line of f.message.split('\n')) log(line)
    }

    // Local git merge drivers (per-clone config; .gitattributes alone is inert)
    try {
      const indexDrv = spawnSync('git', ['config', '--get', 'merge.atlas-index.driver'], {
        cwd: repoRoot,
        encoding: 'utf8',
      })
      const zoneDrv = spawnSync('git', ['config', '--get', 'merge.atlas-zone.driver'], {
        cwd: repoRoot,
        encoding: 'utf8',
      })
      const hasIndex = indexDrv.status === 0 && String(indexDrv.stdout || '').trim()
      const hasZone = zoneDrv.status === 0 && String(zoneDrv.stdout || '').trim()
      if (hasIndex && hasZone) {
        log('✓ merge-driver: atlas-index + atlas-zone configured (local clone)')
      } else if (hasIndex || hasZone) {
        log(
          '⚠ merge-driver: partial — run `atlas wire merge-driver --write` (per-clone; report-first without --write)',
        )
      } else {
        log(
    ...
    // Optional agentic-sage peer (fail-open if sage not installed)
    try {
      const which = spawnSync('sh', ['-c', 'command -v sage'], {
        encoding: 'utf8',
        timeout: 1500,
      })
      const sageBin = which.status === 0 ? String(which.stdout || '').trim() : ''
      if (sageBin) {
        const adapter = path.join(repoRoot, '.agentic-sage', 'adapter.mjs')
        if (existsSync(adapter)) {
          log('✓ agentic-sage: adapter present (.agentic-sage/adapter.mjs)')
        } else {
          log(
            'ℹ agentic-sage: on PATH — optional adapter missing (examples/with-agentic-sage)',
          )
        }
      }
    } catch {
    ...
    // Two-tier package freshness inventory (registry + wired detail)
    freshnessReport = computePackageFreshness(repoRoot, config, {
      fetchLatest: opts.fetchLatest,
      migrations: opts.migrations,
      skillsDir,
    })
    if (freshnessReport.registry.lag) {
      log(
        `⚠ registry: memory-atlas ${freshnessReport.registry.latest} available (installed ${freshnessReport.installed})`,
      )
    } else if (freshnessReport.policy.registry && freshnessReport.registry.latest) {
      log(`✓ registry: installed ${freshnessReport.installed} is current (latest ${freshnessReport.registry.latest})`)
    } else if (freshnessReport.policy.registry) {
      log('ℹ registry: latest unknown (offline or cache empty)')
    }
    if (freshnessReport.wired.pendingCount > 0 && state?.atlasVersion === installed) {
      // already logged pending migrations above when version lags; only extra if needed
    }

    if (strict && freshnessReport && shouldExitNonZero(freshnessReport, true)) {
      stderr.write('atlas doctor: strict — package freshness issues present\n')
      return 1
    }
    return 0
  } catch {
