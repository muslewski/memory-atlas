...
export function runDoctor(argv = [], opts = {}) {
  ...
  try {
    ...
    // Local git merge driver for map/index.md (per-clone config)
    try {
      const drv = spawnSync('git', ['config', '--get', 'merge.atlas-index.driver'], {
        cwd: repoRoot,
        encoding: 'utf8',
      })
      if (drv.status === 0 && String(drv.stdout || '').trim()) {
        log('✓ merge-driver: merge.atlas-index.driver configured')
      } else {
        log('ℹ merge-driver: not installed — run `atlas wire merge-driver` (per-clone)')
      }
    } catch {
      log('ℹ merge-driver: not installed — run `atlas wire merge-driver` (per-clone)')
    }
