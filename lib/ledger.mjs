/**
 * The ledger linter: walks `specs/` and `plans/` (recursively) and checks
 * each note's frontmatter against SPEC.md §3 (universal fields) and §4
 * (per-type lifecycles).
 *
 ...
 * @param {{ zoneSlugs?: Set<string>, folders?: { specs?: string, plans?: string } }} [opts]
 ...
 *   for the two sections this linter walks, resolved through
 *   `config.folders` by callers — defaults to the standard `specs`/`plans`
 *   layout when absent.
...
export function lintLedger(vaultDir, opts = {}) {
  ...
  for (const sectionDir of [folders.specs, folders.plans]) {
