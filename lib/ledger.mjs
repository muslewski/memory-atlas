 ...
 * @param {{ zoneSlugs?: Set<string> }} [opts] existing zone ids, for the
 *   `zones:` entry check — an empty/absent set means every non-empty
 *   `zones:` entry is flagged, which is the correct conservative behavior.
...
export function lintLedger(vaultDir, opts = {}) {
  ...
  for (const sectionDir of ['specs', 'plans']) {
