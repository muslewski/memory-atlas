 ...
 * parser — resist the temptation to grow it into a general one.
 *
 * Supported subset:
...
/**
 * Like setFrontmatterField, but inserts `key: value` before the closing fence
 * when the key is absent. Used by stamp for optional fields like `updated`
 * that hostile/legacy cards may omit.
 *
 * @param {string} text
 * @param {string} key
 * @param {string} value
...
export function upsertFrontmatterField(text, key, value) {
  try {
    return setFrontmatterField(text, key, value)
  } catch (err) {
    if (!(err instanceof Error) || !/not found in frontmatter/.test(err.message)) throw err
  }
  const lines = text.split('\n')
  let start = -1
  let end = -1
  ...
  if (start === -1 || end === -1) {
    throw new Error('upsertFrontmatterField: no frontmatter fence found')
  }
