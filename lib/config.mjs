...
export const DEFAULT_CHECK = {
  ...
  ownership: true,
  /**
   * Fail `atlas check` when the on-disk (working-tree) map/index.md differs
   * from a fresh render. Does not read git HEAD — an uncommitted rebuild that
   * matches the render passes. Set false in repos where many agents recollect
   * in parallel and one integrator rebuilds the index.
   */
  indexSync: true,
  corpus: { enabled: false, maxSummaryLen: 500 },
  /**
   * Two-tier package freshness (fleet follow):
   * - wired: installed package vs .atlas-state atlasVersion / migrations
   * - registry: installed vs npm latest (TTL-cached, fail-open)
   * mode "warn" (default) never fails gate; "fail" or CLI --strict can.
   */
  packageFreshness: {
    mode: 'warn',
    registry: true,
...
/**
 * skills.dir — repo-relative directory for optional vendored copies.
 * skills.nav — entry-ramp skill name.
 * skills.vendorInRepo — when true, always copy package skills into skills.dir
 *   even if the same skill is already installed at user scope
 *   (`ATLAS_USER_SKILLS_DIR` or `~/.claude/skills`). Default false: a
 *   user-scope install satisfies wiring and wire will not re-vendor.
 */
...
/**
 * Optional companion package (`memory-atlas-visuals`) — digests / gallery under
 * the vault. Off by default; paths are vault-relative. Config only: this package
 * never loads or requires the companion (zero runtime deps stay intact).
 */
...
/**
 * Local-first debug telemetry — OFF by default in published package.
 * Fleet enables via ~/.config/memory-atlas/config.json or ATLAS_TELEMETRY=1.
 * Repo atlas.config.json may set enabled:false to opt out; enabled:true is ignored
 * (telemetry can only be turned on per-machine, never by a committed repo file).
 */
...
export function loadConfig(repoRoot, opts = {}) {
  ...
  let text
  try {
    // Refuse FIFOs/sockets/dirs before readFileSync — a FIFO blocks forever
    // (stat is non-blocking; isFile() is false for FIFO/socket/dir).
    const st = fs.statSync(configPath)
    if (!st.isFile()) {
      warn('atlas.config.json is not a regular file — using defaults')
      return base
    }
