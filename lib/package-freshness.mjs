...
export function computePackageFreshness(repoRoot, config, opts = {}) {
  ...
  const skillsDir =
    ...
    (typeof config.skills?.dir === 'string' && config.skills.dir
      ? config.skills.dir
      : '.claude/skills')
  // Default install is user-scope (~/.claude/skills); only vendorInRepo:true
  // still points at the in-repo tree (skills.dir).
  const updateSkillPath =
    config.skills?.vendorInRepo === true
      ? `${skillsDir}/atlas-update/SKILL.md`
      : '~/.claude/skills/atlas-update/SKILL.md'
  ...
  if (wired.lag && wired.messages.length > 0 && wired.messages[0].includes('atlas-update skill')) {
    wired.messages[0] =
      `⬆ atlas ${installed} installed, wired ${wired.wired} — run the atlas-update skill (${updateSkillPath})`
  }
